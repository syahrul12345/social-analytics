...
def getSubscriber(subreddit):
	...
	url = "https://subredditstats.com/api/subreddit?name={}".format(subreddit)
	r = requests.get(url)
	dataInfo = r.json()["subscriberCountTimeSeries"]
	for data in dataInfo:
		try:
			subscriberAxis.append(int(data['count']))
			date = int(data['utcDay'] * 86400)
			#unixDate = time.mktime(datetime.strptime(date,'%Y-%m-%d').timetuple())
			dateAxis.append(datetime.fromtimestamp(date))
...
def plot(subreddit):
	...
	fig.autofmt_xdate()
	fig.savefig("{}.png".format(subreddit))
